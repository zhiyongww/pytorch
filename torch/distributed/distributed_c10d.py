...
def _new_process_group_helper(
    ...
    if "," not in str(backend) and ":" not in str(backend):
        ...
        if backend == Backend.UNDEFINED:
            # Currently when backend is UNDEFINED, both ``gloo`` and ``nccl``
            # backends will be created, we use nccl as default backend so we can
            # correctly call getDefaultBackend which in ProcessGroup.
